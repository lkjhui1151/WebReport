...
def report_web(burp, nmap):
    fileฺBurp = burp
    fileNmap = nmap

    # <--Burp-->
    csvBurp = r'../backend/media/file/{}'.format(fileฺBurp)
    jsonBurp = r'../backend/api/sources/dataBrup.json'

    # <--Nmap-->
    csvNMAP = r'../backend/media/file/{}'.format(fileNmap)
    jsonNMAP = r'../backend/api/sources/dataNmap.json'

    doc = DocxTemplate(
        "../backend/api/sources/templates/templateNessusweb.docx")
